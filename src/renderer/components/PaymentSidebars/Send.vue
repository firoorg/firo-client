<template>
  <section class="send-zcoin-form" :class="privateOrPublic + '-send'">
    <div v-scrollable>
      <form class="send">
        <div class="grid">
          <div class="form">
            ...
            <p v-else class="description">
              No one will know the origin of Zcoin you send privately. Note that
              you may only send multiples of 0.05 XZC.
            </p>
              ...
              <div class="field">
                ...
                <div class="control">
                  <input
                    ...
                    tabindex="1"
                    :placeholder="
                      $t('send.public.detail-public-send.placeholder__label')
                    "
                  />
                </div>
              </div>
              ...
              <div class="field">
                ...
                <div class="control">
                  <input
                    ...
                    tabindex="2"
                    :placeholder="
                      $t('send.public.detail-public-send.placeholder__address')
                    "
                  />
                </div>
                <div
              ...
              <div class="field amount-field">
                ...
                <div class="amount-available">
                  <div v-if="coinControlSelectedAmount > 0">
                    {{ convertToCoin(coinControlSelectedAmount) }} XZC selected
                    in coin control for {{ privateOrPublic }} send.
                  </div>
                  {{ convertToCoin(availableBalance) }} XZC available for
                  {{ privateOrPublic }} send.
                  <div v-if="unavailableBalance > 0">
                    ({{ convertToCoin(unavailableBalance) }} XZC in pending,
                    {{ privateOrPublic === "private" ? "public" : "private" }},
                    and locked balances unavailable.)
                  </div>
                </div>
              </div>
            </fieldset>
          ...
          <div class="buttons">
            ...
            >
              ...
              <div v-else-if="sendPopoverStep === 'complete'" class="ok-icon">
                ✓
              </div>
...
export default {
  ...
  computed: {
    ...
    amountToReceive() {
      return this.subtractFeeFromAmount
        ? this.satoshiAmount - this.transactionFee
        : this.satoshiAmount;
    },
  ...
  beforeMount() {
    ...
    this.$validator.extend("zcoinAddress", {
      ...
      validate: (value) =>
        isValidAddress(value, this.network) || isValidPaymentCode(value),
    });

    this.$validator.extend("amountIsWithinAvailableBalance", {
      // this.availableXzc will still be reactively updated.
      getMessage: () =>
        this.coinControlSelectedAmount == 0
          ? "Amount Is Over Your Available Balance of " +
            convertToCoin(this.availableBalance)
          : "Amount Is Over Your Selected Amount of " +
            convertToCoin(this.coinControlSelectedAmount),
      validate: (value) =>
        (this.coinControlSelectedAmount == 0 &&
          convertToSatoshi(value) <= this.availableBalance) ||
        (this.coinControlSelectedAmount > 0 &&
    ...
    this.$validator.extend("publicAmountIsValid", {
      ...
      validate: (value) =>
        Number(value) !== 0 && !!value.match(/^\d+(\.\d{1,8})?$/),
    });
    ...
    this.$validator.extend("privateAmountDoesntViolateInputLimits", {
      getMessage: () =>
        ...
        `${convertToCoin(
          this.maxPrivateSend
        )} XZC in one transaction. Try minting a larger denomination.`,
  ...
  methods: {
    ...
    async maybeShowFee() {
      ...
      if (this.privateOrPublic === "private") {
        p = $daemon.calcPrivateTxFee(
          this.satoshiAmount,
          this.subtractFeeFromAmount
        );
      } else {
        p = $daemon.calcPublicTxFee(
          this.satoshiAmount,
          this.subtractFeeFromAmount,
          this.txFeePerKb
        );
    ...
    async selectCustomInputs() {
      this.$store.commit(
        "ZcoinPayment/ENTERED_SEND_AMOUNT",
